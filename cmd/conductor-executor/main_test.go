...
func validWorkerEnvironment(t *testing.T) map[string]string {
	...
	return map[string]string{
		...
		"CONDUCTOR_EXECUTION_PROFILES_FILE": filepath.Join(t.TempDir(), "credentials.json"),
	}
}
func TestWorkerConfigurationRequiresExplicitLocalModeAndScope(t *testing.T) {
	for _, row := range []struct{ name, key, value string }{
		...
		{"missing_credentials", "CONDUCTOR_EXECUTION_PROFILES_FILE", ""},
		{"relative_credentials", "CONDUCTOR_EXECUTION_PROFILES_FILE", "credentials.json"},
		{"credential_path_too_long", "CONDUCTOR_EXECUTION_PROFILES_FILE", "/" + strings.Repeat("a", 4096)},
...
func TestWorkerConfigurationSupportsLiteralLoopbackAndNoProviderOverrides(t *testing.T) {
	for _, address := range []string{"", "127.0.0.1:7233", "127.0.0.2:17001", "[::1]:7233", "[::ffff:127.0.0.1]:7233"} {
		t.Run(address, func(t *testing.T) {
			...
			if got.address != wantAddress || got.namespace != "synthetic-context" || got.profilesFile != env["CONDUCTOR_EXECUTION_PROFILES_FILE"] {
				t.Fatalf("worker configuration changed explicit binding: %+v", got)
			}
			wantKeys := map[string]bool{"DATABASE_URL": true, "CONDUCTOR_TEMPORAL_ADDRESS": true, "CONDUCTOR_TEMPORAL_NAMESPACE": true, "CONDUCTOR_EXECUTION_PROFILES_FILE": true, "CONDUCTOR_TEMPORAL_MODE": true, "CONDUCTOR_TEMPORAL_SERVER_NAME": true, "CONDUCTOR_TEMPORAL_CA_FILE": true, "CONDUCTOR_TEMPORAL_CLIENT_CERT_FILE": true, "CONDUCTOR_TEMPORAL_CLIENT_KEY_FILE": true}
...
func TestSDKLoggerDropsMessagesStructuredFieldsAndErrorCauses(t *testing.T) {
	...
	want := "Temporal worker warning; inspect coordination observations\nTemporal worker error; inspect coordination observations\n"
