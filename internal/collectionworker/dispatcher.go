...
type DispatchStore interface {
	...
	ClaimContextDispatch(context.Context, string, string) (*store.ContextDispatch, error)
	FinishContextDispatch(context.Context, store.ContextDispatch, string, string) error
	ContextObservationTargets(context.Context) ([]store.ContextObservationTarget, error)
	ObserveContextExecution(context.Context, string, string, string, string, string, string, string) error
}
...
func (d *Dispatcher) Step(ctx context.Context) (bool, error) {
	...
	item, err := d.store.ClaimContextDispatch(ctx, d.target, d.namespace)
	if err != nil || item == nil {
		return false, err
	}
	if item.Mismatch {
	...
	if execution := item.Work.Collection.Execution; execution != nil && execution.State == "unresolved" {
		...
		return true, d.store.FinishContextDispatch(ctx, *item, "unresolved", "execution_unresolved")
	}
	// PostgreSQL completion remains authoritative even after Temporal history
	// expires. Never restart a collection merely to recreate execution history.
	if item.Work.Collection.Receipt != nil && item.Operation == "start" {
		return true, d.store.FinishContextDispatch(ctx, *item, "delivered", "receipt_recorded")
	}
	ref := contextworkflow.Reference{ID: item.ID, Binding: item.Binding}
	id := contextworkflow.WorkflowName + "/" + item.ID
	...
	if errors.Is(lookupErr, contextworkflow.ErrNotFound) {
		if knownRun != "" || item.UncertaintyExpired {
...
func runID(work store.CollectionWork) string {
	if work.Collection.Execution != nil {
		return work.Collection.Execution.RunID
	}
	return ""
}

func (d *Dispatcher) dispatchFailure(ctx context.Context, item store.ContextDispatch, err error, run string) error {
...
func (d *Dispatcher) finish(ctx context.Context, item store.ContextDispatch, outcome, code, state, run string) error {
	...
	if err := d.store.ObserveContextExecution(ctx, item.ID, item.Binding, item.BoundTarget, item.BoundNamespace, contextworkflow.WorkflowName+"/"+item.ID, run, state); err != nil {
		return err
	}
	return d.store.FinishContextDispatch(ctx, item, outcome, code)
}
...
func (d *Dispatcher) checkedState(ctx context.Context, ref contextworkflow.Reference, observation contextworkflow.Observation) (string, error) {
	...
	work, err := d.store.CollectionWork(ctx, ref.ID, ref.Binding)
	if err != nil {
		return "", err
	}
	r := work.Collection.Receipt
	if r == nil || observation.Result == nil || observation.Result.ReceiptID != ref.ID || observation.Result.ReceiptID != r.ID || observation.Result.Digest != r.Digest {
...
func (d *Dispatcher) Reconcile(ctx context.Context) error {
	...
	items, err := d.store.ContextObservationTargets(ctx)
	if err != nil {
		return err
	}
	for _, item := range items {
		...
		if err = d.store.ObserveContextExecution(ctx, item.ID, item.Binding, item.BoundTarget, item.BoundNamespace, id, run, state); err != nil {
			return err
		}
	}
	return nil
