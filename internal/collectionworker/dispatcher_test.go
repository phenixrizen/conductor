...
type dispatchDB struct {
	item                                                         *store.ContextDispatch
	work                                                         store.CollectionWork
	targets                                                      []store.ContextObservationTarget
	outcome, code, state, run, observedTarget, observedNamespace string
	finishErr, observeErr                                        error
	claims                                                       int
}

func (f *dispatchDB) CollectionWork(context.Context, string, string) (store.CollectionWork, error) {
	return f.work, nil
}
func (f *dispatchDB) ClaimContextDispatch(context.Context, string, string) (*store.ContextDispatch, error) {
	f.claims++
	return f.item, nil
}
func (f *dispatchDB) FinishContextDispatch(_ context.Context, _ store.ContextDispatch, outcome, code string) error {
	f.outcome, f.code = outcome, code
	return f.finishErr
}
func (f *dispatchDB) ContextObservationTargets(context.Context) ([]store.ContextObservationTarget, error) {
	return f.targets, nil
}
func (f *dispatchDB) ObserveContextExecution(_ context.Context, _, _, target, namespace, _, run, state string) error {
...
func dispatcherFixture(t *testing.T) (*Dispatcher, *dispatchDB, *dispatchRuntime) {
	...
	db := &dispatchDB{item: &store.ContextDispatch{ID: id, Binding: binding, Operation: "start", BoundTarget: target, BoundNamespace: "synthetic", Work: store.CollectionWork{Binding: binding, Collection: domain.Collection{ID: id}}}}
	...
	for _, name := range []string{"known history missing", "unknown history expired", "changed binding", "unresolved acknowledgment lost", "receipt survives retention", "process target changed"} {
		t.Run(name, func(t *testing.T) {
			...
			case "known history missing":
				db.item.Work.Collection.Execution = &domain.CollectionExecution{RunID: "old-run", State: "running"}
			...
			case "unresolved acknowledgment lost":
				db.item.Work.Collection.Execution = &domain.CollectionExecution{State: "unresolved"}
			case "receipt survives retention":
				db.item.Work.Collection.Receipt = &domain.CollectionReceipt{ID: db.item.ID, Digest: strings.Repeat("f", 64)}
			...
			if name == "receipt survives retention" {
				if runtime.lookups != 0 || db.outcome != "delivered" {
...
func TestDispatcherChecksReceiptAndKeepsUnknownProgressExplicit(t *testing.T) {
	for _, name := range []string{"matching receipt", "missing receipt", "wrong digest", "wrong receipt", "unavailable", "lost history", "wrong target"} {
		t.Run(name, func(t *testing.T) {
			...
			db.work.Collection.Receipt = &domain.CollectionReceipt{ID: db.item.ID, Digest: digest}
			db.targets = []store.ContextObservationTarget{{ID: db.item.ID, Binding: db.item.Binding, RunID: "known-run", BoundTarget: d.target, BoundNamespace: d.namespace}}
			runtime.lookupErr = nil
			...
			case "missing receipt":
				db.work.Collection.Receipt = nil
			case "wrong digest":
