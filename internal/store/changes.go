...
func (p *Postgres) List(ctx context.Context, repository, before string, limit int) (domain.ChangePage, error) {
	...
	rows, err := p.pool.Query(ctx, `SELECT c.id,r.revision,r.digest,r.author,c.created_at,
		...
		ORDER BY c.created_at DESC,c.id DESC LIMIT $4`, repository, beforeTime, beforeID, limit+1)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&change.ID, &change.Revision, &change.Digest, &change.Author, &change.CreatedAt, &change.Approved, &change.Repository); err != nil {
