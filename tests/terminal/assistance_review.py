...
def main():
    ...
    try:
        ...
        # The ten-line owner-supplied mark never consumes the factual header or
        # the six reserved body rows. Detailed 80x24 views use the plain heading.
        since = len(author.output)
        author.resize(80, 24)
        author.wait_text("Conductor | principal", since)
        author.settle()
        assert "/_______/" not in author.text(since)
        assert "Digest:" in author.text(since)
        ...
        press(author, "h", "Returned to Change")
        press(author, "b", "Shared work loaded")
        since = len(author.output)
        author.resize(80, 24)
        author.wait_text("/_______/", since)
        author.settle()
        Path("/tmp/conductor-terminal-brand-browse.txt").write_text(author.text(since))
